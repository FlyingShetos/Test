...
try {
  ...
  // ================= BEAT BATTLE MODE (ONLINE RHYTHM DUEL) =================
  run(`beatMode = false; snowballMode = false; startBeatMode();`);   // no auth/db in harness: flag flips, matchmaking politely bails
  check('beat mode goes online with a flag', run(`beatMode === true`), run('game_mode'));
  run(`game_mode = 'online_match'; beatMode = true; reset_game(); game_mode = 'online_match'; myOnlineRole = 'guest'; activeMatchID = null; online_round = 1; resetBeatState();`);
  check('chart is generated, long and time-sorted', run(`beatNotes.length === 240 && beatNotes.every((n, i) => i === 0 || n.t > beatNotes[i-1].t) && beatNotes.every(n => n.lane >= 0 && n.lane < 4 && n.t >= 3000)`));
  check('same seed = identical chart on both clients', run(`JSON.stringify(beatGenerateChart(777)) === JSON.stringify(beatGenerateChart(777))`));
  check('different rounds = different charts', run(`JSON.stringify(beatGenerateChart(777)) !== JSON.stringify(beatGenerateChart(778))`));
  ...
  stepFrames(30);
  check('beat duel has no pipes and no bird physics', run(`pipes.length === 0 && bird.y === 300 && myHasCrashed === false`), 'y=' + run('bird.y'));
  run(`beatChartStart = performance.now() - beatNotes[0].t; beatTap(beatNotes[0].lane);`);
  check('tap on the beat lands a hit', run(`beatHits === 1 && beatCombo === 1 && beatNotes[0].hit === true`), 'hits=' + run('beatHits'));
  run(`beatChartStart = performance.now() - (beatNotes[0].t + 10); beatTap((beatNotes[1].lane + 2) % 4);`);
  check('flailing at an empty lane is a miss', run(`beatMisses === 1 && beatCombo === 0`), 'misses=' + run('beatMisses'));
  run(`resetBeatState(); window.roundEnding = false; beatChartStart = performance.now() - (beatNotes[0].t + 400); updateBeatBattle();`);
  check('notes that drift past un-tapped count as misses', run(`beatMisses >= 1 && beatNotes[0].missed === true`), 'misses=' + run('beatMisses'));
  run(`resetBeatState(); beatHits = 20; beatMisses = 0; beatEnemyHits = 0; beatEnemyMisses = 0; window.roundEnding = false;`);
  check('bar reaches my edge at net +20', run('beatBarMine() === 100'));
  run(`checkRoundResult();`);
  check('pushing the bar across wins the round', run('window.roundEnding === true'));
  run(`beatHits = 0; beatEnemyHits = 20; window.roundEnding = false; checkRoundResult();`);
  check('rival pushing the bar across loses the round', run('window.roundEnding === true'));
  run(`beatHits = 7; beatEnemyHits = 4; updateOnlineUI(true);`);
  ...
  run(`resetBeatState(); window.roundEnding = false; beatChartStart = performance.now() - beatNotes[0].t;`);
  run(`Got_Player_Input({ type: 'mousedown', clientX: (beatNotes[0].lane * 120) + 60, clientY: 300, button: 0, preventDefault() {}, target: { tagName: 'CANVAS', closest: () => null } });`);
  check('screen taps map to the right lane', run('beatHits === 1'), 'hits=' + run('beatHits'));
  // ---- cross-device visibility: phone must see the PC rival on-screen ----
  run(`bird.y = CANVAS_HEIGHT / 2;`);
  check('my position is sent as a screen fraction, not raw pixels', run('birdYToWire() === 500'), run('birdYToWire()'));
  run(`bird.y = CANVAS_HEIGHT - 14;`);
  check('rival hugging their floor lands on MY floor, not below it', run(`enemyGhostYFromWire(birdYToWire()) === CANVAS_HEIGHT - 70`), run('enemyGhostYFromWire(birdYToWire())').toFixed(1));
  check('a tall-PC pixel overflow gets clamped on screen', run(`enemyGhostYFromWire(5000) === CANVAS_HEIGHT - 70 && enemyGhostYFromWire(0) === 10`));
  check('mid-screen positions survive the round trip', Math.abs(run(`enemyGhostYFromWire(500)`) - run('CANVAS_HEIGHT') / 2) < 1);
  ...
  run(`beatMode = false; snowballMode = false; game_mode = 'running'; reset_game(); game_mode = 'running'; currentSkinKey = 'witch';`);
